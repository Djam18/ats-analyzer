import enum, uuid
from sqlalchemy import Boolean, Column, Enum, Integer, Text, String, CheckConstraint, Index, ForeignKey
...
class ApplicationStage(str, enum.Enum):
    received="received"; shortlisted="shortlisted"; hr_interview="hr_interview"
    tech_interview="tech_interview"; offer="offer"; hired="hired"
 
class EducationLevel(str, enum.Enum):
    not_specified="not_specified"; bac2="bac2"; bac3="bac3"; bac5="bac5"; phd="phd"
...
class Application(Base):
    ...
    job_posting_id  = Column(UUID(as_uuid=True), ForeignKey("job_posting.id", ondelete="CASCADE"), nullable=False)
    ...
    global_score    = Column(Integer, CheckConstraint("global_score BETWEEN 0 AND 100", name="ck_score_range"))
    ...
    resume_file       = relationship("ResumeFile",       back_populates="application", uselist=False, cascade="all, delete-orphan")
    resume_extraction = relationship("ResumeExtraction", back_populates="application", uselist=False, cascade="all, delete-orphan")
    parsing_jobs      = relationship("ParsingJob",       back_populates="application", cascade="all, delete-orphan")
    action_logs       = relationship("ActionLog",        back_populates="application", cascade="all, delete-orphan")
    internal_notes    = relationship("InternalNote",     back_populates="application", cascade="all, delete-orphan")
    email_logs        = relationship("EmailLog",         back_populates="application", cascade="all, delete-orphan")
 
class ResumeFile(Base):
    __tablename__ = "resume_file"
    id             = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    application_id = Column(UUID(as_uuid=True), ForeignKey("application.id", ondelete="CASCADE"), nullable=False, unique=True)
